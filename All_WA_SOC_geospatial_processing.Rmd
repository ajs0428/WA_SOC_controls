...
lf_hoh <- crop(lf_wa_rcl, hoh_studyarea_4326, mask = TRUE) |>
  ...
  mask(mask = hoh_chm, filename = "SOIL CARBON/All_WA/data/Rasters/hoh_LANDFIRE.tif", overwrite = TRUE)
lf_mas <- crop(lf_wa_rcl, mas_studyarea_4326, mask = TRUE)|>
  project("EPSG:2856") |>
  resample(y = mas_chm, method = "near") |> 
  mask(mask = mas_chm, filename = "SOIL CARBON/All_WA/data/Rasters/mashel_LANDFIRE.tif", overwrite = TRUE)
lf_col <- crop(lf_wa_rcl, col_studyarea_4326, mask = TRUE) |>
  project("EPSG:2855") |>
  resample(y = col_chm, method = "near") |> 
  mask(mask = col_chm, filename = "SOIL CARBON/All_WA/data/Rasters/colville_LANDFIRE.tif", overwrite = TRUE)


lf_hoh_classes <- levels(lf_hoh)[[1]] |> as_tibble() |> 
  left_join(y = lf_classes, by = join_by(ID == values)) |> select(-EVT) |> 
  mutate(names = case_when(
    str_detect(names, pattern = fixed("spruce", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("hemlock", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("fir", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("pine", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("conifer", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("shrub", ignore_case = T)) ~ "Shrubland",
      str_detect(names, pattern = fixed("sagebrush", ignore_case = T)) ~ "Shrubland",
      str_detect(names, pattern = fixed("grass", ignore_case = T)) ~ "Herbaceous",
      str_detect(names, pattern = fixed("Tsuga", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("larch", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("Oak", ignore_case = T)) ~ "Deciduous",
      str_detect(names, pattern = fixed("Deciduous", ignore_case = T)) ~ "Deciduous",
      str_detect(names, pattern = fixed("Evergreen", ignore_case = T)) ~ "Conifer",
      str_detect(names, pattern = fixed("Mixed", ignore_case = T)) ~ "Mixed",
      str_detect(names, pattern = fixed("Sparse", ignore_case = T)) ~ "Mixed",
      str_detect(names, pattern = fixed("Aspen", ignore_case = T)) ~ "Deciduous",
      str_detect(names, pattern = fixed("Mahogany", ignore_case = T)) ~ "Deciduous",
      str_detect(names, pattern = fixed("Savanna", ignore_case = T)) ~ "Herbaceous",
      str_detect(names, pattern = fixed("Riparian", ignore_case = T)) ~ "Mixed",
      str_detect(names, pattern = fixed("swamp", ignore_case = T)) ~ "Mixed",
      str_detect(names, pattern = fixed("Volcanic", ignore_case = T)) ~ "Mixed",
      str_detect(names, pattern = fixed("Herbaceous", ignore_case = T)) ~ "Herbaceous",
    .default = names))

lf_hoh_names <- categories(lf_hoh, layer=1, value = lf_hoh_classes, active = 1)
coltab(lf_hoh_names) <- NULL
plot(lf_hoh_names)
```

```{r}
lf_hoh <- rast("SOIL CARBON/All_WA/data/Rasters/hoh_LANDFIRE.tif")
lf_mas <- rast("SOIL CARBON/All_WA/data/Rasters/mashel_LANDFIRE.tif")
lf_col <-rast("SOIL CARBON/All_WA/data/Rasters/colville_LANDFIRE.tif")
```
...
list.files("SOIL CARBON/All_WA/data/Rasters", 
                         pattern = "Colville", full.names = TRUE,
                         ignore.case = TRUE,
                         include.dirs = FALSE) |> 
              str_subset(pattern = "^(?!.*flood)") |>
              str_subset(pattern = "^(?!.*aux.xml)") |>
